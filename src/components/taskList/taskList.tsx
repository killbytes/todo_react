...
function TaskList(props: TaskListProps) {
    ...
    const filterTasks = useMemo(() => props.tasks.filter(it=>{
        if (props.filter === 'all') return true
        if (props.filter === 'active') return !it.isCompleted
        if (props.filter === 'complited') return it.isCompleted
        return false
    }), [props.filter, props.tasks])

    return (
        <>
            {filterTasks.map(it => <Task key={it.id} task={it} setTasks={props.setTasks}/>)}
        </>
    );
}
